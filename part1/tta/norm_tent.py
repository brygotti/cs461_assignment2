...
class NormTent(TTAMethod):
    ...
    def __init__(
        ...
        ), "overall batch_size must be >= norm_batch_size"
        assert (
            batch_size % norm_batch_size == 0
        ), "overall batch_size must be divisible by norm_batch_size"
    ...
    def forward(self, x):
        ...
        for _ in range(self.tent_steps):
            outputs = tent.forward_and_adapt(x, self.model, self.tent_optimizer)
