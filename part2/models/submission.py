...
class Submission(nn.Module):
    ...
    def train_with_cv(
        ...
        for fold, (train_ids, val_ids) in enumerate(kfold.split(indices)):
            ...
            for epoch in range(num_epochs):
                ...
                if patience_counter >= early_stopping_patience:
                    ...
                    print(
                        f"Best validation accuracy for this fold: {best_fold_val_acc:.4f}"
                    )
            ...
            if best_fold_val_acc > best_val_acc:
                ...
                if save_best_fold:
                    ...
                    print(
                        f"Saved best fold model (Fold {best_fold}) with Val Acc: {best_val_acc:.4f}"
                    )
        ...
        print(f"{'='*50}")
        print(
            f"Mean Train Accuracy: {np.mean(cv_results['fold_train_acc']):.4f} ± {np.std(cv_results['fold_train_acc']):.4f}"
        )
        print(
            f"Mean Val Accuracy: {np.mean(cv_results['fold_val_acc']):.4f} ± {np.std(cv_results['fold_val_acc']):.4f}"
        )
        print(
            f"Mean Train Loss: {np.mean(cv_results['fold_train_loss']):.4f} ± {np.std(cv_results['fold_train_loss']):.4f}"
        )
        print(
            f"Mean Val Loss: {np.mean(cv_results['fold_val_loss']):.4f} ± {np.std(cv_results['fold_val_loss']):.4f}"
        )
